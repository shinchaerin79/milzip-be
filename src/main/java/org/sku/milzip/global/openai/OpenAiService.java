...
public class OpenAiService {
  ...
  /** 텍스트를 OpenAI text-embedding-3-small 모델로 임베딩 벡터로 변환합니다. */
  ...
  /**
   * 사용자 쿼리와 후보 매장 목록을 GPT에 전달하여 추천 순위와 이유를 반환합니다.
   *
   * @return storeId → reason 매핑 리스트 (GPT 추천 순서)
   */
  public List<AiRankResult> rankStores(
      String userQuery,
      String companion,
      String course,
    ...
    try {
      ...
      String userMessage = buildUserMessage(userQuery, companion, course, storesContext);

      List<Map<String, String>> messages =
          List.of(
              Map.of("role", "system", "content", buildSystemPrompt(maxRecommendations)),
      ...
      throw new RuntimeException("GPT 추천 생성 실패: " + e.getMessage(), e);
    } catch (Exception e) {
  ...
  private String buildSystemPrompt(int maxRecommendations) {
    return """
        당신은 군장병 혜택 매장 추천 전문가입니다.
        사용자의 상황과 요청에 가장 잘 맞는 매장을 후보 목록에서 최대 %d개 선택해 추천하세요.
        같은 지역(시/도)의 매장들을 우선적으로 묶어서 추천하세요.
        각 추천에는 사용자의 상황과 연관된 구체적이고 친절한 추천 이유를 한국어로 작성하세요.
  ...
  private String buildUserMessage(
      String freeText, String companion, String course, String storesContext) {
    ...
    if (course != null && !course.isBlank()) {
      sb.append("목적/코스: ").append(course).append("\n");
    }
